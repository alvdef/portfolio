...
* **section** (string): The main X-axis category. This value is NOT hardcoded — it is derived automatically from the top-level directory name in the Obsidian vault that contains the file (e.g., a file at `coding/my-project.md` has `section: "coding"`). The frontmatter field is therefore redundant for routing and MAY be omitted; the build script MUST infer `section` from the file's directory path.
...
2. **Pre-Build Sync:** A deterministic pre-build script scans the vault's top-level directories, enumerates them as the section list, emits `src/generated/sections.ts`, then copies `.md` files into `src/content/` preserving directory structure. The build MUST fail if this step is skipped or if the output is stale.
