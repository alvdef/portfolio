...
async function fetchVideoDetails(apiKey: string, videoIds: string[]) {
  const details = new Map<
    string,
    { title: string; uploadedAt: string; views: number; channelTitle: string; channelId: string }
  >();
  ...
  for (const ids of chunk(videoIds, 50)) {
    ...
    for (const item of payload.items ?? []) {
      ...
      if (!videoId || !title || !uploadedAt || Number.isNaN(views) || !channelTitle || !channelId) continue;
      details.set(videoId, { title, uploadedAt, views, channelTitle, channelId });
    }
  }
